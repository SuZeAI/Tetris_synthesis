...
class GetBest(object):
    ...
    @staticmethod
    def choose(initialField, piece, next_piece, offsetX, weights, parent, id, linesent):
...
class Agent(object):
    ...
    def choose_action(self, observation):
        if len(self.list_action) != 0:
            ...
            linesent = self.get_Sentline(observation)
            x = 4
            crr_piece = self.get_next_piece(copy.deepcopy(observation), 6)
            next_piece = self.get_next_piece(copy.deepcopy(observation), 1)
            GetBest.choose(board, crr_piece, next_piece, x, self.weight, self, crr_piece[0][0], linesent)
