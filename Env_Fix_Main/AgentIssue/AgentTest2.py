...
class GetBest(object):
    ...
    def best(field, workingPieces, workingPieceIndex, weights, level, workingIdPieces):
        ...
        for rotation in range(0, shapes_rotation[workingIdPiece]):
            for offset in range(0, field.width):
                ...
                if not result is None:
                    ...
                    if workingPieceIndex == len(workingPieces) - 1:
                        ...
                        _, _, score = GetBest.best(field, workingPieces, workingPieceIndex + 1, weights, level + 1, workingIdPieces)
    ...
    @staticmethod
    def choose(initialField, piece, next_piece, offsetX, weights, parent,  idPieceCrr, idNextPiece):
...
class Agent(object):
    ...
    def choose_action(self, observation):
        if len(self.list_action) != 0:
            ...
            x = 4
            crr_piece, idPieceCrr = self.get_next_piece(copy.deepcopy(observation), 6), self.getIdPiece(copy.deepcopy(observation), 6)
            next_piece, idNextPiece = self.get_next_piece(copy.deepcopy(observation), 1), self.getIdPiece(copy.deepcopy(observation), 1)

            GetBest.choose(board, crr_piece, next_piece, x, self.weight, self, idPieceCrr, idNextPiece)
