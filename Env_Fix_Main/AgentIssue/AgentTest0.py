...
class GetBest(object):
    ...
    @staticmethod
    def best(field, workingPieces, workingPieceIndex, weights, level, idPiece):
        bestRotation = None
        bestOffset = None
        bestScore = None
        workingPieceIndex = copy.deepcopy(workingPieceIndex)
        workingPiece = workingPieces[workingPieceIndex]
        workingIndex = idPiece[workingPieceIndex]
        # shapes_rotation = {6 : 4, 5 : 2, 4 : 2, 2 : 4, 3 : 4, 0 : 2, 1 : 1}
        shapes_rotation = { 0 : 2, 1 : 1,  2 : 4, 3 : 4, 4 : 2, 5 : 2, 6 : 4}

        # flat_piece = [val for sublist in workingPiece for val in sublist]

        for rotation in range(0, shapes_rotation[workingIndex]):
            for offset in range(0, field.width):
                ...
                if not result is None:
                    ...
                    if workingPieceIndex == len(workingPieces) - 1:
                        ...
                        _, _, score = GetBest.best(field, workingPieces, workingPieceIndex + 1, weights, level + 1, idPiece)
    ...
    @staticmethod
    def choose(initialField, piece, next_piece, offsetX, weights, parent, id, idPiece):
        field = Field(len(initialField[0]), len(initialField))
        field.updateField(copy.deepcopy(initialField))
        offset, rotation, _ = GetBest.best(field, [piece, next_piece], 0, weights, 1, idPiece)
        moves = []
...
class Agent(object):
    ...
    def choose_action(self, observation):
        if len(self.list_action) != 0:
            ...
            # print(np.array(np.array(observation).squeeze()[:20, 10:17], dtype="float16"))
            ...
            crr_piece = self.getNextPiece(copy.deepcopy(observation), 6)
            next_piece = self.getNextPiece(copy.deepcopy(observation), 1)
            idPiece = [self.getIdPiece(copy.deepcopy(observation), 6), self.getIdPiece(copy.deepcopy(observation), 1)]
            ...
            GetBest.choose(board, crr_piece, next_piece, x_piece, self.weight, self, crr_piece[0][0], idPiece)
